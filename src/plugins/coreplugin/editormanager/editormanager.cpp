...
IEditor* EditorManagerPrivate::openEditorWith(const QString& fileName, Core::Id editorId)
{
    ...
    QList<IEditor*>    editorsOpenForFile = DocumentModel::editorsForFilePath(fileName);
...
void EditorManagerPrivate::saveSettings()
{
    ...
    Qt::CaseSensitivity defaultSensitivity = OsSpecificAspects::fileNameCaseSensitivity(HostOsInfo::hostOs());
...
void EditorManagerPrivate::readSettings()
{
    ...
    {
        d->m_warnBeforeOpeningBigFilesEnabled = qs->value(warnBeforeOpeningBigTextFilesKey).toBool();
...
bool EditorManager::closeAllDocuments()
{
    ...
    const QList<DocumentModel::Entry*> entriesToClose = Utils::filtered(DocumentModel::entries(), Utils::equal(&DocumentModel::Entry::pinned, false));
...
TextFileFormat::LineTerminationMode EditorManager::defaultLineEnding()
{
    ...
    const int  defaultLineTerminator = settings->value(Constants::SETTINGS_DEFAULT_LINE_TERMINATOR,
                                                      TextFileFormat::LineTerminationMode::NativeLineTerminator)
                                          .toInt();
